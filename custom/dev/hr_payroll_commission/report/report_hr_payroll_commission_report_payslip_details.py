...
class ReportHrPayrollCommunityReportPayslipDetails(models.AbstractModel):
    ...
    _name = 'report.hr_payroll_commission.report_com_payslipdetails'
    ...
    def get_lines_by_contribution_register(self, payslip_lines):
        ...
        for line in payslip_lines.filtered('register_id'):
            result.setdefault(line.com_slip_id.id, {})
            result[line.com_slip_id.id].setdefault(line.register_id, line)
            result[line.com_slip_id.id][line.register_id] |= line
        for payslip_id, lines_dict in result.items():
            res.setdefault(payslip_id, [])
    ...
    def _get_report_values(self, docids, data=None):
        ...
        payslips = self.env['hr.commission.payslip'].browse(docids)
        return {
            'doc_ids': docids,
            'doc_model': 'hr.commission.payslip',
            'docs': payslips,
            'data': data,
